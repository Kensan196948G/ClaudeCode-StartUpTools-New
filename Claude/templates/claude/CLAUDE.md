...
- 実際のデプロイは**人間（ユーザー）が手動**で実行する（CTOはデプロイを自動実行しない）
...
実行主体・自律度・観測可能性が異なる **3 つの運用モード** を明示的に並置する。
`state.json` の `operation_mode.current` で現在モードを宣言する。

| モード | 用途 | 実行主体 | Agent 実装 | 観測 |
|---|---|---|---|---|
| 🤖 `cron_autonomous` | 月〜土 自律ループ（無人） | Linux cron | SubAgent 12 役割（§6） | GitHub Projects + ログ |
| 💬 `manual_subagent` | 小〜中規模の対話セッション | 人間 (CTO) | SubAgent 順次起動 | 端末ログ |
| 🎼 `conducted` | 大型変更・障害復旧・複数 Issue 並走 | 人間 CTO + Claude リード | 公式 Agent Teams + Agent View | TUI 一画面 |

#### モード選択ガイド

- **既定は `cron_autonomous`** — `state.json:operation_mode.current` 未設定時は cron モード扱い
- 手動セッションで簡易タスクなら `manual_subagent`
- 以下のいずれかに該当する場合のみ `conducted` を選択:
  - 同時に 3 Issue 以上を並走させたい
  - Verify を真の並列で 30 分以内に収束させたい
  - 障害復旧で複数役割の同時介入が必要
  - 人間 CTO が端末に在席して指揮できる

#### モード固有制約

| モード | 主要制約 |
|---|---|
| `cron_autonomous` | 5h 厳守、5h 到達時 §15 必須処理、teammates 起動禁止 |
| `manual_subagent` | §13 トークン制御、§6 SubAgent vs Agent Teams 判定 |
| `conducted` | ローカル端末必須、`CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1` 必須、4h 上限推奨（5h 強制終了より早く graceful shutdown） |

詳細リファレンス: `.claude/claudeos/system/conducted-mode.md`
...
> ⚠️ **名称の重要な注意**: 本書の「Agent Teams」は **ClaudeOS v8 の役割定義 (12 役割)** を指す。
> これは既定で **SubAgent ベース実装**（Task tool / メイン Claude が逐次・並列で SubAgent を起動）。
> Claude Code 公式の experimental 機能 `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1`（リード+teammates、独立セッション）は
> **別実装** であり、`operation_mode.current="conducted"` 時のみ利用する。両者は§3 三層運用モデルで分離している。
...
### conducted モード固有ルール (`operation_mode.current="conducted"` 時のみ)

公式 Agent Teams + Agent View 併用時の追加規約。SubAgent ベース運用には適用されない。

| 項目 | ルール |
|---|---|
| 起動条件 | `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1` かつ人間 CTO 在席かつローカル端末 |
| spawn 方式 | **on-demand**。フェーズ別に必要な teammate のみ起動（全員起動禁止） |
| 同時チーム数 | 1 リード = 1 チーム（公式制約）。複数 Issue 並走は §10 WorkTree と組み合わせる |
| skills / MCP | teammates に伝播しない → `.claude/claudeos/scripts/teamspawn/spawn-<role>.sh` で各 teammate を bootstrap（雛形） |
| teammates 進捗 | リードが**毎フェーズ終了時**に `state.json:agent_teams.teammates[]` へミラーする |
| 残時間ガード | 残 60 分未満で teammate 新規 spawn 禁止。残 30 分で§15 graceful shutdown 強制発火 |
| トークンガード | §13 で 70% 超過したら teammate 新規 spawn 禁止 |
| 復元 | session resume で teammates 復元不可 → state.json から spawn 計画のみ復元 |
...
### Agent View 監視（conducted モード時のみ）

`operation_mode.current="conducted"` で複数 WorkTree を並走させる場合、
Claude Code 公式の **Agent View (v2.1.139+ デフォルト有効)** を観測卓として用いる。

| 手順 | コマンド / 操作 |
|---|---|
| Agent View 起動 | `claude agents`（別端末で開く） |
| teammate 状態確認 | TUI 上で Working / Needs input / Completed / Failed を一覧 |
| Peek パネル | スペースキー → 最新出力確認、Enter で接続 / ← で離脱 |
| 介入 | リード CTO 経由でメッセージを送る（teammate へ直接介入は最終手段） |
| 無効化 | 必要時のみ `disableAgentView=true` または `CLAUDE_CODE_DISABLE_AGENT_VIEW=1` |

#### 利用前提

- 🟢 人間 CTO が端末に在席している時間帯のみ有効。`cron_autonomous` モードでは使用しない（ローカル TUI のため）
- 🟡 マシンスリープ時に teammate セッションが停止する → セッション維持設定を行うこと
- 🔴 セッション再開時に teammates は復元されない → 重要状態は §15 graceful shutdown で永続化
...
> 本セクションを **graceful shutdown** と定義する。
> OS シャットダウンでも Claude Code プロセス強制終了 (`/exit`, `Ctrl+C`) でもなく、
> **Claude Code セッションが下記 9 ステップを完走してから自然終了するアプリ層手続き** を指す。
...
| トリガ | 条件 |
|---|---|
| 能動 (時間) | 経過 ≥ 270 分（5h − 30 分）でリード CTO が自発移行 |
| 受動 (hook) | `.claude/settings.json:Stop` hook が `session-end.js` を発火 |
| 緊急 | §13 token 95% 到達 / §12 Auto Repair 上限 / Blocked 検知 |

### conducted モード追加処理（teammates 対応）

`operation_mode.current="conducted"` 時のみ、9 ステップに以下を**前段挿入**する:

| 順 | 内容 | 実装 |
|---|---|---|
| (前) | 全 teammates の進捗を回収 | リードが各 teammate に commit & push 指令 |
| (前) | teammate 別 worktree の commit を強制 | 失敗時は WIP commit で残す |
| (前) | `state.json:agent_teams.teammates[]` へ snapshot | `session-end.js` が自動実行 |
| (前) | Memory MCP に「再開時 spawn 計画」を記録 | リード CTO が要約 → MCP 保存 |
| 1-9 | 既存の必須処理を実行 | 通常どおり |
| (後) | teammates 達成状況テーブルを最終報告に含める | リードが集約 |

teammates 復元は session resume で**不可** (公式制約)。次セッションでは `state.json:agent_teams.teammates[]` を読んで **spawn 計画のみ復元**する。
