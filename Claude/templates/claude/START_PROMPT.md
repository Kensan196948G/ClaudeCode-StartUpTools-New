...
## 🔒 ステップ A: /goal を **逐語実行**（最優先・改変禁止）

以下のコードブロック内の `/goal "..."` 文字列を **一字一句変えずに** Skill ツール経由で即時実行すること。
...
> ⚠️ 実行後、SessionStart hook (`verify-goal-set.js`) が必須キーワード（CTO全権委任 / Monitor / Verify / AgentTeams / 5時間 / README / GitHub Projects / stop after）の存在を検証する。1 つでも欠落すると警告が出るので、その場合は本ブロックを再度逐語実行すること。
